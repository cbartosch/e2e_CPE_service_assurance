...
8. **Six subgraphs were swept for the first time on 2026-08-24, and half of every sweep survived.**
   ...
   The other five subgraphs' **37 survivors are recorded at their own module's denominator, not the
   suite's**, and the difference matters: on the one module where both numbers exist, the suite
   caught 30% of what the module missed. So the 37 is an upper bound on the gaps and the true figure
   is likely nearer 26. **None of the five are closed, and the per-survivor re-check against the
   whole suite is not finished** — it is a 37-run job and this pass did not wait for it. What is
   settled is that each of the five has survivors at all, which is what takes the *mutation-checked*
   mark off those rows.
