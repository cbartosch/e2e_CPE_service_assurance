...
Measured on **2026-08-23**, after the last `PENDING_STAGES` entry was retracted: `ruff check src
tests` passes, `ruff format --check src tests` passes at **135 files already formatted**, `mypy
--strict src/lpr_cpe` reports no issues in **109** source files, and `pytest` collects and passes
**918** tests at **85.28%** line coverage, so `make lint`, `make test` and `make check` are all
green.

The revisions before this one were dated 2026-08-18 (124 / 103 / 853 / 82.77%) and 2026-08-17
...
| tests | 918 passing | unit only; no integration, contract or scenario tests yet, and none of the 17 required scenarios exist. This row read 916 on 2026-08-21 and was one out at the time — see the note under the opening paragraph |
| coverage | **85.28%**, gate is 85% | **the gate is met and `make test` is green**, measured 2026-08-23 by running that target's own command, which prints `Required test coverage of 85% reached. Total coverage: 85.28%` and exits 0. It was 82.77% and failing on 2026-08-18 and 85.29% on the 21st. Nothing has been written to raise it: the 2.52 points that cleared the bar were `jtrack/simulator.py` going 33% to 72.28% as the plant stages drove it, and the hundredth of a point lost since is a documentation pass adding one test that executes no new source. **0.28 points of headroom is not a margin** — one uncovered module of any size would spend it, so a green `make test` here says the gate passed today and nothing about whether it will tomorrow |
...
1. **The committed suite is unit-only, and the coverage gate is now met — which is the least
   ...
   tests rather than on a throwaway script, but all **918** are unit tests. There are no
   integration, contract or scenario tests and none of the 17 required scenarios exist. Coverage is
   **85.28%** against the 85% bar, measured 2026-08-23, so `make test` and `make check` are green
