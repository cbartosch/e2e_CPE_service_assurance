...
8. **Six subgraphs were swept for the first time on 2026-08-24, and half of every sweep survived.**
   ...
   **That re-check has since been run for all 37, and it caught nothing.** 0 of 37. The estimate in
   ...
   So the 37 is not an upper bound with slack in it. **It is 37 defects, and this pass closed 13.**

   | subgraph | survivors | closed | left |
   | --- | --- | --- | --- |
   | `reconciliation_closure` | 6 | **6** | 0 |
   | `plant_execution` | 7 | **7** | 0 |
   | `plant_referral` | 6 | 0 | 6 |
   | `preventive_maintenance` | 9 | 0 | 9 |
   | `field_planning` | 9 | 0 | 9 |

   Of the 13 closed, twelve were real gaps now held by named tests, each watched red. The
   thirteenth is a **proven equivalent mutant**: replacing `plant_report`'s non-mapping refusal with
   `answer = {}` cannot differ, because an empty mapping falls through to `MRStatus("")`, raises
   `ValueError` and returns `None` by the next line. That was predicted in the test's docstring
   before the sweep confirmed it, which is the only way an equivalence claim is worth anything.

   `plant_referral`, `preventive_maintenance` and `field_planning` keep their survivor counts and
   nothing else. Their §5 rows say so.
