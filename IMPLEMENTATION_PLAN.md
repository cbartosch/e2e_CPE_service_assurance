...
behind the same Protocol and exercised only when a key is present. **Built as described**, in
`models/`, and two things about it are worth carrying forward. `FakeModelProvider` hashes with
`blake2b` rather than `hash()`, because Python's string hash is salted per process and a
`hash()`-keyed fake would answer differently on every run — irreproducible in the one way the fake
exists to prevent. And one of its four canned answers is deliberately **not JSON**, because
`write_narrative`'s re-ask and its templated fallback are both unreachable if every answer parses.
...
issues in **111** source files, and `pytest` collects and passes **985** tests at **85.67%** line
...
| `persistence` checkpointer + serde | done | **The allowlist was applied by the factory and by nothing that used it until 2026-08-25.** 21 of the 23 `InMemorySaver(...)` constructions in the suite were bare, so every stage test resumed through the permissive default serde — exactly what this row’s own `build_memory_checkpointer` docstring calls “the local and **test** backend” was written to prevent. Measured: one incident driven to closure outside pytest emitted **45** `Deserializing unregistered type` warnings, covering `IncidentStatus`, `ApprovalRequest`, `WorkOrder` and 42 others, every one of them on the allowlist and none reaching it; through the factory it emits zero and reaches the same `closed`. All 21 now use the factory, and two guards hold it in both directions — no test may build a saver by hand, and an exemption that stops being needed fails. | 14 committed tests, each paired with a control that fails; lazy Postgres import checked in a clean subprocess; the Postgres branch driven without a database and the shipped defect reinstated to watch it fail |
| `persistence` outbox + migrations | done | 19 tests, five mutations watched red. `outbox.py` holds the event, two stores behind one Protocol, and a relay; `migrations.py` plus `migrations/0001_persistence.sql` hold the schema. **The interesting result is negative and it is measured rather than assumed: the outbox cannot commit in the same transaction as the checkpoint.** `AsyncPostgresSaver.aput` is called by the framework *after* a node returns and opens its own cursor, so there is no hook that runs inside it — by the time the transaction exists, the node that wanted to enqueue has finished. The saver's connection is reachable as `saver.conn` and a subclass could insert on the same cursor; not taken, because it binds this system to the internals of a checkpointer it does not own and `_cursor(pipeline=True)` puts psycopg in pipeline mode. So the boundary is one step wider and OUTBOX-1 names it: a crash between the checkpoint and `flush()` loses the record of an intent no external system saw, and the replay stages it again under the same idempotency key. **Jitter is a hash of the event id, not `random`** — it decorrelates different events, which is what jitter is for, while leaving two relays contending for one row in agreement about when it is due, and the schedule assertable. `WriteGate` stages every authorised action including the simulated ones, so the outbox in simulation is the record of what would have been sent. Postgres store and `apply_all` are unexercised — OUTBOX-2 — and five of the seven tables are schema only, OUTBOX-3 |
| `api` | done | 25 committed tests, driven against a real graph with no mocks — the two defects the package is written against are both invisible to a mocked one. All 13 endpoints the specification names, `make serve` runs uvicorn, and the two measured hazards are held by tests: `/state` reads through `graph.inspect` because a paused *nested* subgraph's writes are not in the parent's state (four of six gates are nested, and the naive read reports `diagnosing` for an incident on a supervisor's queue), and `/resume` refuses an empty mapping because `Command(resume={})` resumes nothing and would return 200 having done nothing. RBAC is checked at the boundary, where the pending request's `kind` is readable, so a wrong role never becomes an `ApprovalDecision`. Every response is redacted. **Five gaps are recorded rather than papered over**: no incident index so no listing and no webhook routing (API-1), in-process delivery suppression that a restart forgets (API-2), no metric store so `/metrics` publishes the vocabulary and says so (API-3), no product tier on the wire (API-4), and no scheduler, so the API cannot release a stability window and cannot close an incident (API-6) |
| model provider + deterministic fake | done | 17 tests. `models/` holds the Protocol, the two providers and `narrative.write_narrative`, which is the only model-assisted function in the system. D6 is enforced structurally — `NarrativeDraft` declares neither `verdict` nor `wifi_health_score`, so a model cannot return a number a decision reads, and `extra="forbid"` makes that a rejection rather than an intention. **The test that earns its place reads the schema text rather than the field set**, and that is what caught the real defect: pydantic renders each class docstring into the generated schema as `description`, and those docstrings argue at length about why `verdict` and `wifi_health_score` are absent. So the prompt was carrying both forbidden names, the string `D6`, and several hundred characters of internal reasoning — 2587 characters down to 1991 once `_without_prose` stripped them. A test asserting "we ignore those keys" would have passed throughout. `GraphContext.model` distinguishes *omitted* from *explicitly `None`* with a private sentinel, because a plain `None` default conflated them and made the templated path unreachable through the public constructor — which is the path A5 makes load-bearing. `AnthropicModelProvider` is unexercised: a test would need a key and a network. Gap MODEL-1 |
...
| `runner.py` + `lpr-cpe run` | done | 19 committed tests. Answers all five `interrupt()` shapes by payload key, and `PAUSE_SHAPES` is held complete in both directions against the keys read back out of `src` — an unanswered shape falls through to an approval, which the parser rejects in silence, and that is the harness defect this section already records having cost a sweep its conclusions. Two defects were found by running it: `run` as an `add_subparsers` entry broke `lpr-cpe topology` outright, because a subparser and an optional positional compete for the first token; and the production-writes refusal named `APP_MODE` when the variable is `LPR_APP_MODE` — verified with the unprefixed name, the guard did not fire and the run closed an incident. **The suite’s closure count moved because of this command, not because of the graph**: swept through it, 3 of the 41 fixtures close and 38 escalate, against the 1 a walk answering two shapes had measured |
| tests | 985 passing | unit only; no integration, contract or scenario tests yet, and none of the 17 required scenarios exist. Read the live figure off `audit/MANIFEST.json`, not this cell |
...
| demo | **runs** | `make demo` is `lpr-cpe run SVC-UT-001-B-01`, which drives one incident from event to closure against the simulators and answers all five pause shapes itself. **This is not the seventeen specification scenarios**, which now live in `tests/scenarios/`: one scripted path is not seventeen named ones, and the answers it gives are a script rather than a simulation of an operator. What it did buy is that the workflow is runnable at all — before it, six approval gates paused and nothing but a test harness could resume them |
| the seventeen specification scenarios | done | 24 tests in `tests/scenarios/`, driven through the real parent graph over the committed fixtures -- no node mocked, no subgraph substituted. **14 of the 17 reach their specified outcome and the other three say why they do not**, each pinned by an assertion so a fixture change that closed one turns the suite red rather than quietly widening coverage nobody notices. All six shortfalls are fixture-set limitations rather than code ones, recorded as SCENARIO-1/2/3/10/14/16 in `docs/vendor-integration-gaps.md`. Three source mutations confirm the suite is load-bearing: making `truck_roll_count` count rows instead of records fails scenarios 1, 9, 10 and 11; removing D18's completeness gate fails scenario 8 alone, which is the one whose whole claim is that an incomplete packet blocks the MR. **The third mutation is the one worth recording** -- reinstating the `append_revision` doubling defect leaves all 23 of the original scenario tests green, because they read `truck_roll_count` and `mr_attempt_count`, which collapse revisions by id and answer 1 either way. That is the right thing for them to assert, and it is exactly why a 49,152-entry channel went unnoticed for as long as it did, so a 24th test guards raw channel size at the level the defect appeared. Scenarios 6, 7 and 9 seed one rejected hypothesis to run at all (EXEC-1, confirmed here across all twelve fixtures that reach a Clean Boots dispatch); scenario 8 needs no seed, because the specification's failure case is this system's default and its success case is the one that needs help |
...
| Docker Compose | **written, build unverified** | `docker-compose.yml` (Postgres + the API) and a `Dockerfile`. `docker compose config` validates. **`docker compose build` has never succeeded here**: it fails at `pip install` with `CERTIFICATE_VERIFY_FAILED` against pypi.org, a TLS-inspecting proxy whose root CA the container does not trust. `--trusted-host` would make it pass and would ship an image installing dependencies unverified, so it was not added — gap DOCKER-1, and the CI `compose` job is where it is first proven. The stack runs the **simulation** profile with a real database on purpose: durable checkpoints, simulated adapters, which is the pairing that makes gap 5 testable without inventing a vendor endpoint |
| CI | **exists, unproven** | `.github/workflows/ci.yml`, three jobs. `gates` runs `python -m lpr_cpe.audit` — the same six commands in the same order as a local run, so CI cannot drift from `make check` — and uploads the bundle on failure as well as success, because the bundle is most useful for the run that failed. `postgres` stands a real server up and closes the provable half of gap 5: the scope opens, `setup()` runs its DDL and the saver answers `aget_tuple`. It is `continue-on-error` because the `@pytest.mark.postgres` tests it would run are **not written**, so a red there today would be red for having nothing to run. `compose` builds the image and asserts `/ready`. **None of the three has ever executed**: this repository has no runs yet, and a workflow file is a claim until a run makes it a measurement |
...
7. **No gate reads the prose, and the prose had drifted.** Every check in this repository runs
   ...
   - README's Layout table listed `src/lpr_cpe/api/`, which did not exist then and does now; described `persistence/`
